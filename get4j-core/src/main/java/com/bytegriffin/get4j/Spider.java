...
public class Spider {
	...
	/**
	 * 设置抓取url <br>
	 * 必填项。表示要抓取的Url，如果抓取模式pageMode为list_detail，该值为列表Url， 其中可变的页数PageNum需要用大括号{}括起来
	 *
	...
	/**
	 * 设置页面变化探测器，用于探测抓取页面的变化，如果有变化就立刻抓取，没有变化则继续探测。 默认是探测整个页面是否发生改变。
	 *
	 * @return Spider
	 */
	 ...
	 *
	 * @param detailLinkSelector 详情页面选择器
	 * @return Spider
	 */
	public Spider detailLinkSelector(String detailLinkSelector) {
		seed.setFetchDetailLinkSelector(detailLinkSelector);
		return this;
	}
	...
	/**
	 * 将解析的结果保存到Mysql中<br>
	 * jdbc格式：<a>jdbc:mysql://localhost:3306/spider?useSSL=false&amp;serverTimezone=UTC&amp;characterEncoding=UTF-8&amp;user=root&amp;password=root</a>
	 *
	...
	/**
	 * 将解析的结果保存到MongoDB中<br>
	 * <a>mongodb://localhost:27017</a> 或者加密
	 * <a>mongodb://user1:pwd1@host1/?authSource=db1&amp;ssl=true</a>
	 * 或者多个<a>mongodb://host1:27017,host2:27017</a>
	 *
	...
	private Spider getAnnotation(Class<? extends PageParser> clazz) {
		...
		for (Annotation an : ans) {
			...
			if ("ListDetail".equalsIgnoreCase(type) && clazz.isAnnotationPresent(ListDetail.class)) {
				...
				this.detailLinkSelector(seed.detailSelector());
	...
	public void start() {
		...
		SpiderEngine.create().setClusterNode(clusterNode).setSeed(seed).setResourceSync(resourceSync)
				.setConfiguration(configuration).setDynamicField(dynamicField).build();
	}
	...
	public static void main(String... args) {
		...
		SpiderEngine.create().setSeeds(seeds).setResourceSync(synchronizer).setConfiguration(configuration)
				.setDynamicFields(dynamicFields).build();
		logger.info("爬虫开始启动...");
	}
